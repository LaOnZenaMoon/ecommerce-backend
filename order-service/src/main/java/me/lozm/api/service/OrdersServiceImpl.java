...
public class OrdersServiceImpl implements OrdersService {
    ...
    @Override
    public List<OrdersInfoVo> getOrdersList(OrdersSearchVo searchVo) {
        return ordersRepository.getOrderList(searchVo);
    }
    ...
    public OrdersCreateResponseVo createOrders(OrdersCreateRequestVo ordersCreateRequestVo) {
        ...
        if (!product.canBeOrdered(orderedQuantity)) {
            throw new IllegalArgumentException(format("주문하신 상품 수량만큼 상품을 주문할 수 없습니다. " +
                            "상품 ID: %s, 주문한 상품 수량: %d, 남은 상품 수량: %d",
